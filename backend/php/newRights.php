...
class new_rights extends linkBd{
		...
		function updete_admin(){
			...
			if($admin){
				...
				$result = mysqli_query($link, "UPDATE `user rights` SET  `admin` = '1' , user = '0' WHERE `email` = '$admin' ");
			}
			else{
		...
		function delete(){
			...
			$admin = $admin['email'];
			if($admin){
				$result = mysqli_query($link, "DELETE FROM `user rights` WHERE email = '$admin' ");
		...
		function updete_user(){
			...
			$user = $user['email'];
			if($user){
				$result = mysqli_query($link, "UPDATE `user rights` SET  `admin` = '0' , user = '1' WHERE `email` = '$user' ");
				echo "жопа";
			}
			else{
