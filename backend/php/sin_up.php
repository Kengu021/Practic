...
class Autoris extends linkBd{
	...
	public function Sin_in(){
		...
		$query = "INSERT INTO `user acounts` VALUES(NULL,'$name','$password','$email')";
		$rights = mysqli_query($link, "INSERT INTO `user rights` VALUES(NULL, '$email', NULL, '1')");
		...
		if ($result and $rights){
				...
				header('Location: http://pabunen/main.php');//Сюда воткнешь ссылку на страницу
				exit();
		}
				...
				and isset($_POST['name']))
		{
			...
			if($password == $password_rep){
				...
				if ($len >= 8 and $len <= 16){
					...
					$email = $_POST['email'];
					$result = mysqli_query($link,"SELECT * FROM `user acounts` WHERE email LIKE '$email' ");
